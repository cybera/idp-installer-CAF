...
setJavaHome () {
		...
		ln -s /usr/java/latest /usr/java/default
                ...
                for i in `ls $JAVA_HOME/bin/`; do rm -f /var/lib/alternatives/$i;update-alternatives --install /usr/bin/$i $i $JAVA_HOME/bin/$i 1; done
        fi
...
maven2Path=`basename ${maven2File}  -bin.tar.gz`
	...
	then
		...
		if [ ! -f ${downloadPath}/${maven2File} ]; then
...
{
	if [ "${dist}" == "centos" -a "${redhatDist}" == "7" -o "${dist}" == "ubuntu" -a "${debianDist}" == "14" ]; then
		${whiptailBin} --backtitle "${GUIbacktitle}" --title "Centos7 detected!" --msgbox "Tomcat6 available for Centos6 only! Choose OK to return to main menu" ${whipSize}
		return
	fi
...
invokeShibbolethUpgradeProcess()
{
		...
		if [ ${dist} == "ubuntu" ]; then
			apt-get -y remove --purge tomcat* openjdk* default-jre
		else
