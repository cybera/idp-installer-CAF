...
#cdinu
                  eduroamTask=$(${whiptailBin} --backtitle "${GUIbacktitle}" --title "Identity Server Main Menu" --cancel-button "exit" --menu --clear  -- "Which do you want to do?" ${whipSize} 5 refresh "Refresh relevant CentOS packages" review "Review install Settings" installEduroam "Install only the eduroam service" installFedSSOTomcat6 "Install only Federated SSO service (Tomcat6)"  installFedSSOJetty9 "Install only Federated SSO service (Jetty9)" installAll "Install eduroam and Federated SSO services" Upgrade "Upgrade to Jetty" 3>&1 1>&2 2>&3)
		...
		elif [ "${eduroamTask}" = "installFedSSOTomcat6" ]
		then
			...
			if echo "${installer_section0_buildComponentList}" | grep -q "shibboleth"; then
					...
					invokeShibbolethInstallProcessTomcat6
                ...
                elif [ "${eduroamTask}" = "installFedSSOJetty9" ]
                then
